import numpy as np
from bokeh.documents import Document
...
figures = {Plane.TRANSVERSE: None, Plane.SAGITTAL: None, Plane.CORONAL: None}
...
def get_image_data(series_data: np.ndarray, plane: Plane, index: int):
    image = np.take(series_data, index, axis=plane.value)
...
def fix_index(series_data: np.ndarray, plane: Plane, index: int):
    axis_size = series_data.shape[plane.value]
    if index >= axis_size:
        index = 0
    elif index < 0:
        index = axis_size - 1
...
def create_displayed_values_toggle():
    ...
    displayed_values_toggle.callback = create_toggle_callback("displayed_values")
...
def update_crosshair_source(plane: Plane, vertical_line: list, horizontal_line: list):
...
def update_crosshairs(skip: Plane = None):
    for plane in figures:
        if plane is not skip and isinstance(get_image_from_source(plane), np.ndarray):
            update_crosshair(plane)


def add_wheel_interaction(plane: Plane):
    figures[plane].on_event(MouseWheel, partial(handle_mouse_wheel, plane=plane))
...
def create_index_slider(series_data: np.ndarray, plane: Plane) -> Slider:
    slider = Slider(
        start=0,
        end=series_data.shape[plane.value] - 1,
        value=0,
        step=1,
...
def create_visibility_checkbox():
    visibility_checkbox = CheckboxButtonGroup(labels=CHECKBOX_LABELS, active=[0, 2])
...
def create_crosshair_color_select():
    select = Select(
        title="Crosshair Color", value=DEFAULT_CROSSHAIR_COLOR, options=crosshair_colors
    )
...
for plane in figures:
    ...
    range_slider.on_change("value", partial(change_displayed_range, plane=plane))
...
main_layout = row(
    ...
    column(
        ...
        widgetbox(*index_sliders.values(), css_classes=["plane_indices", "hidden"]),
        create_displayed_values_toggle(),
        widgetbox(*range_sliders.values(), css_classes=["displayed_values", "hidden"]),
        extra_widgets,
    ),
